  ...
  try {
    ...
    if (sb) {
      ...
      if (!error) {
        ...
        // NOTE: Halal Passport points for a review are awarded when it is
        // APPROVED by moderation (app/api/admin/queue reviews→approve), NOT here
        // — otherwise spam/pending reviews could farm points. We only set
        // reviews.user_id above so the award can find the reviewer on approval.
