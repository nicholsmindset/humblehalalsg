...
import { getSeoPageByLocation } from "@/lib/seo-pages";
import { SITE } from "@/lib/seo";

export const size = { width: 1200, height: 630 };
export const contentType = "image/png";
export const alt = "Halal food in Singapore";

export default async function OgImage({ params }: { params: Promise<{ location: string }> }) {
  const { location } = await params;
  const p = getSeoPageByLocation(location);
  const h1 = p?.h1 ?? "Halal Food in Singapore";
