...
import { HOME_FAQ, VERIFY_FAQ, BUSINESS_FAQ } from "@/lib/faq";
...
export const metadata = pageMeta({ title: "Frequently asked questions — Humble Halal", description: "Answers about finding halal places in Singapore, our trust badges and verification, Ask AI, TikTok, Halal Passport and business features.", path: "/faq" });

export default async function Page() {
  const flags = await getServerFlags();
  const featureFaqs = HELP.filter((h) => !h.flag || flags[h.flag]).flatMap((h) => h.faqs);
  const allFaqs = [...HOME_FAQ, ...VERIFY_FAQ, ...BUSINESS_FAQ, ...featureFaqs];
  return (
    <>
