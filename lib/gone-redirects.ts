/* Humble Halal — relevant 301 targets for gone content.
   ...
   unpublished, the route resolves a target here and records a durable redirect
   so the URL 308s (in middleware) to a relevant hub instead of 404ing — keeping
   the ranking/link equity Google already gave the URL. Targets are computed LIVE
   from the entity's own category/area; the `redirects` table (Supabase) is the
   durable backstop for hard-deletes and hand-authored redirects.

   NB: distinct from lib/redirects.ts, which is the STATIC build-time flat-URL
   migration map consumed by next.config.ts. This module is the DYNAMIC,
   runtime-gone-content resolver (server-only — pulls the service-role client). */
import "server-only";
