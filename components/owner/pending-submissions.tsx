      ...
      {open && <div className="modal-veil" role="presentation" onMouseDown={(e) => { if (e.target === e.currentTarget) setOpen(false); }}>
        <section className="modal" role="dialog" aria-modal="true" aria-label="Edit pending business" style={{ maxWidth: 720, maxHeight: "90vh", overflowY: "auto" }}>
          ...
          <div className="grid-2 g12">
            <div className="field"><label>Business name</label><input className="input" value={form.name} onChange={(e) => set("name", e.target.value)} /></div>
            ...
            <div className="field"><label>WhatsApp</label><input className="input" value={form.whatsapp} onChange={(e) => set("whatsapp", e.target.value)} /></div>
            <div className="field" style={{ gridColumn: "1 / -1" }}><label>Description</label><textarea className="input" rows={4} value={form.desc} onChange={(e) => set("desc", e.target.value)} /></div>
            <div className="field" style={{ gridColumn: "1 / -1" }}><label>Address</label><input className="input" value={form.address} onChange={(e) => set("address", e.target.value)} /></div>
            <div className="field"><label>Town / area</label><input className="input" value={form.town} onChange={(e) => set("town", e.target.value)} /></div>
            <div className="field"><label>Postal code</label><input className="input" value={form.postal} onChange={(e) => set("postal", e.target.value)} /></div>
            <div className="field"><label>Halal status</label><input className="input" value={form.halal} onChange={(e) => set("halal", e.target.value)} /></div>
            <div className="field"><label>Certificate number</label><input className="input" value={form.certNo} onChange={(e) => set("certNo", e.target.value)} /></div>
          </div>
