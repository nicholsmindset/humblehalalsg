...
export function OwnerCouponsTab({ toast, initialRedeem }: { toast: (m: string) => void; initialRedeem?: string }) {
  ...
  return (
    <div className="dash-pane stack g16">
      ...
      {businesses.length > 1 && <div className="field" style={{ maxWidth: 360 }}><label htmlFor="oc-business">Business</label><select id="oc-business" className="select" value={businessId} onChange={(e) => setBusinessId(e.target.value)}>{businesses.map((b) => <option value={b.id} key={b.id}>{b.name}</option>)}</select></div>}
      ...
      {open && (
        <div className="card coupon-builder">
          ...
          <div className="ed-grid2">
            <div className="field"><label htmlFor="oc-title">Title</label><input id="oc-title" className="input" maxLength={90} value={form.title} onChange={(e) => update("title", e.target.value)} placeholder="Weekday lunch special" /></div>
            <div className="field"><label htmlFor="oc-discount-type">Discount</label><div className="flex g8"><select id="oc-discount-type" className="select" value={form.discountType} onChange={(e) => update("discountType", e.target.value)}><option value="percent">Percent off</option><option value="fixed">Dollar amount off</option><option value="free_item">Free item</option><option value="bundle">Bundle</option></select>{["percent","fixed"].includes(form.discountType) && <input className="input" type="number" min="1" max={form.discountType === "percent" ? 100 : undefined} value={form.discountValue} onChange={(e) => update("discountValue", e.target.value)} />}</div></div>
            {["free_item","bundle"].includes(form.discountType) && <div className="field"><label htmlFor="oc-reward">Reward</label><input id="oc-reward" className="input" value={form.rewardText} onChange={(e) => update("rewardText", e.target.value)} placeholder="Free teh tarik with any main" /></div>}
            <div className="field"><label htmlFor="oc-details">Details</label><input id="oc-details" className="input" value={form.details} onChange={(e) => update("details", e.target.value)} placeholder="Dine-in only" /></div>
            <div className="field"><label htmlFor="oc-min-spend">Minimum spend (SGD)</label><input id="oc-min-spend" className="input" type="number" min="0" step="0.01" value={form.minSpend} onChange={(e) => update("minSpend", e.target.value)} /></div>
            <div className="field"><label htmlFor="oc-starts">Starts</label><input id="oc-starts" className="input" type="date" value={form.startsAt} onChange={(e) => update("startsAt", e.target.value)} /></div>
            <div className="field"><label htmlFor="oc-ends">Ends</label><input id="oc-ends" className="input" type="date" value={form.endsAt} onChange={(e) => update("endsAt", e.target.value)} /></div>
            <div className="field"><label htmlFor="oc-redeem-start">Daily time window (optional)</label><div className="flex g8"><input id="oc-redeem-start" className="input" type="time" value={form.redeemStart} onChange={(e) => update("redeemStart", e.target.value)} /><input className="input" type="time" value={form.redeemEnd} onChange={(e) => update("redeemEnd", e.target.value)} /></div></div>
            <div className="field"><label htmlFor="oc-total">Total claims (blank = unlimited)</label><input id="oc-total" className="input" type="number" min="1" value={form.totalLimit} onChange={(e) => update("totalLimit", e.target.value)} /></div>
          </div>
          ...
          <div className="field"><label htmlFor="oc-terms">Terms</label><textarea id="oc-terms" className="textarea" value={form.terms} onChange={(e) => update("terms", e.target.value)} placeholder="Not valid with other promotions. One redemption per person." /></div>
