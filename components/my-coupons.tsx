...
export function MyCoupons() {
  ...
  useEffect(() => {
    ...
    let alive = true;
    // Lazy-load the QR library only when there are claimed coupons to render,
    // keeping it out of the /deals route's initial bundle.
    (async () => {
      const QRCode = (await import("qrcode")).default;
      for (const row of claimed) {
        try {
          const url = await QRCode.toDataURL(`${window.location.origin}/owner?tab=promotions&redeem=${row.token}`, { width: 180, margin: 1 });
          if (alive) setQrs((q) => ({ ...q, [row.id]: url }));
        } catch { /* ignore */ }
      }
    })();
    return () => { alive = false; };
