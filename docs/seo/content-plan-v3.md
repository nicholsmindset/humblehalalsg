...
| WS-B | Brand pages — **15 web-verified** shipped (56→71). Batch 1: PastaMania, Poulet, Soup Spoon, Namu Bulgogi (cert); Sushi Tei, Oriental Kopi (no-pork); IKEA (partial); Astons (not-cert). Batch 2: 4Fingers, Texas Chicken, Wok Hey (cert); Swensen's (partial); Toast Box, Collin's, Sushi Express (not-cert). Held back (ambiguous sources): Each-a-Cup, Mixue, Gong Cha. | ~20k / KD 0–2 | 🟢 15 done, long tail remains |
| WS-A | Enriched **8** cuisine *pSEO* pages with lookFor + considerations + 4 hand-written FAQ each: Korean, Japanese, Western, Dessert (batch 1); Thai, Dim Sum, Steamboat, Seafood (batch 2). Distinct from legacy blog roundups (pSEO owns the transactional head). | "Optimise" rows | 🟢 8 done; more on the generic template remain |
