...
| WS-B | **All 19 location pages scaffolded** (5 batch-1 malls + 6 batch-2 malls: Downtown East, Great World City, Hillion, Junction 8, Tiong Bahru Plaza, Wisma Geylang Serai + 8 CBD/central districts: City Hall, Chinatown, Tanjong Pagar, Somerset, HarbourFront, Toa Payoh, Raffles Place, Esplanade). Render now, self-noindex until ≥3 listings tagged with each `match` token are seeded. Seed template covers all 19. New districts still need AreaProfiles before indexing. | ~5,000+ / KD 0 | 🟡 Code + seed template shipped; **awaiting listing import** |
